...
1. Run **generate_env_config.py** to automatically generate and store the config JSON file for the desired map
...
1. Create a new python file in `/gym-unrealcv/gym_unrealcv/envs`, and write your environment in this file. You can inheriting the base classes in [base_env.py](./gym_unrealcv/envs/base_env.py), which provide the basic functions for the gym environment. You can also refer to the existing environments in the same folder for more details. Note that you need to write your own reward function in the new environment.

2. Import your environment into the `__init__.py` file of the collection. This file will be located at `/gym-unrealcv/gym_unrealcv/envs/__init__.py`. Add `from gym_unrealcv.envs.{your_env_file} import {Your_Env_Class}` to this file.
3. Register your env in `gym-unrealcv/gym_unrealcv/__init__.py`
...
1. Test your environment by running a random agent
