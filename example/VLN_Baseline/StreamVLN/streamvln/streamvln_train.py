...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        # import ipdb; ipdb.set_trace()
        # input_ids_batch, labels_batch, image_batch, depth_batch, pose_batch, intrinsic_batch, time_ids_batch, inflection_weight_batch = zip(
        #     *[(inst["input_ids"], inst["labels"], inst["image"], inst["depth"], inst["pose"], inst["intrinsic"], inst["time_ids"], inst["inflection_weight"])
        #       for inst in instances]
        # )
        input_ids_batch, labels_batch, image_batch, depth_batch, pose_batch, intrinsic_batch, time_ids_batch, inflection_weight_batch = \
            tuple([instance[key] for instance in instances] for key in ("input_ids", "labels","images","depths","poses","intrinsic","time_ids","inflection_weights"))
        
        # Pad input_ids and labels
        input_ids_batch = self.pad_sequence(input_ids_batch, batch_first=True, padding_value=self.tokenizer.pad_token_id)
        labels_batch = self.pad_sequence(labels_batch, batch_first=True, padding_value=IGNORE_INDEX)
        ...
        # Pad image and depth tensors
        # for idx, sample in enumerate(image_batch):
        #     print(f"Sample {idx}: type = {type(sample)}, keys = {sample.keys() if isinstance(sample, dict) else 'N/A'}")

        # image_ = [torch.tensor(sample["images"]) for sample in image_batch]
        # depth_ = [torch.tensor(sample["depths"]) for sample in depth_batch]
        # image_ = torch.tensor([i for i in image_batch])
        # depth_ = torch.tensor(depth_batch)
        # print("11111111", image_.shape, depth_.shape)
        img_lens = [len(img) for img in image_batch]
        max_len=max(img_lens)
        image_batch = torch.tensor([x + [0] * (max_len - len(x)) for x in image_batch])
        image_batch = self.pad_tensors(image_batch, img_lens)
        depth_batch = self.pad_tensors(depth_batch, img_lens)
        
        # Pad poses and intrinsics
        padded_poses = []
        padded_intrinsics = []
        for pose, intrinsic in zip(pose_batch, intrinsic_batch):
            view = pose.size(0)
            pad_view = max(img_lens) - view
            if pad_view > 0:
                padded_pose = torch.cat([pose, torch.eye(4).unsqueeze(0).repeat(pad_view, 1, 1).to(pose.device)], dim=0)
                padded_intrinsic = torch.cat([intrinsic, intrinsic[0:1].repeat(pad_view, 1, 1).to(intrinsic.device)], dim=0)
            else:
                padded_pose = pose
        ...
        pose_batch = torch.stack(padded_poses)
        intrinsic_batch = torch.stack(padded_intrinsics)
        
        # Pad time_ids and inflection_weights
        time_ids_batch = self.pad_sequence(time_ids_batch, batch_first=True, padding_value=-1)
        inflection_weight_batch = self.pad_sequence(inflection_weight_batch, batch_first=True, padding_value=0)
        ...
        batch = {
            ...
            "weights": inflection_weight_batch
        }
        return batch
