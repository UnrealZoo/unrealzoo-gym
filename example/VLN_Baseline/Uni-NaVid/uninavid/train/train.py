...
from uninavid.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
...
from uninavid.constants import VIDEO_START_SPECIAL_TOKEN, VIDEO_END_SPECIAL_TOKEN, IMAGE_START_TOKEN, IMAGE_END_TOKEN, NAVIGATION_SPECIAL_TOKEN, IAMGE_SEPARATOR,  NAVIGATION_IDENTIFIER
...
def random_color_jitter(video, brightness_range=(0.8, 1.2), contrast_range=(0.8, 1.2), 
                        saturation_range=(0.8, 1.2), hue_range=(-0.1, 0.1), prob=0.05):
                ...
                if 'image' in sources[0]:
                    ...
                    if suffix == 'pkl':
                        ...
                        if  "NAV_ID" in self.list_data_dict[i]['id'] and len(frame_idx) > 1: # TODO: temp fix for nav
                            ...
                            num_frames_to_sample = len(video) - 1 - random.randint(0, max_drop_frames)     
                            assert  num_frames_to_sample >= 0
                            sampled_frame_indices = sorted(random.sample(range(len(video) - 1), num_frames_to_sample))
                            
                            sampled_frame_indices.append(last_frame_index)

                            sampled_frame_indices = duplicate_with_probability(sampled_frame_indices, 0.03)
