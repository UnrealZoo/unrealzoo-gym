...
if __name__ == '__main__':
    ...
    env = agents.NavAgents(env, mask_agent=True) #use nav mesh to control the rest agents(only env.unwrapped.protagonist_id is the tracker)
    episode_count = 1
    rewards = 0
    done = False
        ...
        for eps in range(0, episode_count):
            ...
            while True:
                ...
                if args.render:
                    ...
                    #  img = img[..., ::-1]  # bgr->rgb
                cv2.imshow('show', obs[tracker_id][:,:,:3])
                cv2.waitKey(1)
                if done:
